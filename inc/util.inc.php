...
function dump_array($title,$array)
{
	...
	if (is_array($array)) {
		...
		?><tr><td align=left valign=top><b><code>NO ARRAY!</code></b></td><td></tr><?php
	}
	print "</table>\n";
}
...
function timestamp2relative($timestamp)
{
	if ($t = strtotime($timestamp)) {
		...
		if ($delta < 60)
			return "$prefix $delta Sekunden";
		$delta = round($delta / 60);
		if ($delta < 60)
			return "$prefix $delta Minuten";
		$delta = round($delta / 60);
		if ($delta < 24)
			return "$prefix $delta Stunden";
		$delta = round($delta / 24);
		if ($delta < 31)
			return "$prefix $delta Tagen";
		$delta = round($delta / 31);
		return "$prefix $delta Monaten";
	}
	return '';
}
